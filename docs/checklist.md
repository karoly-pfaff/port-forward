...
- [x] **Slice 3** — macOS LaunchAgent polish: auto-copy from `build/portier/`, `--source-dir`/`--no-start`/`--runtime` options, label bug fixed, `--purge` on uninstall, `build-release.sh` for portable tar.gz, signing/notarization docs, npm lifecycle scripts. Build via `npm run installer:macos`. Validate via `npm run validate:service:macos`.

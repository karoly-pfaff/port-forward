...
- [ ] `npm run validate:contract` — API contract parity: runs all API scenarios against TypeScript server; if Go binary available, runs same scenarios against Go service and compares response shapes, status codes, field names, and error shapes. Includes the `rule-group` scenario group (v1.8): optional rule `group` metadata round-trips through create/PATCH/clear/export-import/plan-apply, invalid groups (too long / control chars) are rejected, ungrouped/legacy rules omit the field, and a group-only plan change is a non-destructive `update` (cross-runtime `plan:GROUP_CHANGE` parity). (`scripts/validate-contract.js`)

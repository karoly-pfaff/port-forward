...
| `api/api.go:configApply` (Fix Slice 7) | the `if len(result.Errors) > 0 { … return }` import-error guard (2 lines) | Defensive Resilience-C invariant. Structurally unreachable from apply: all reachable import errors are pre-blocked by the plan engine (duplicate bindings via `detectDuplicateKeys`, invalid rules via plan validation) or throw (persist → 500); apply only imports in replace mode. The `Handler` holds a concrete `*manager.Manager` (no injection seam), so the guard cannot be hit in a unit test without the deferred manager-interface refactor (SOLID-G/Pattern-G). The **response construction** is unit-tested directly via the extracted `applyImportErrorResponse` helper (`TestApplyImportErrorResponse`); the TS twin (`api.ts`) is covered end-to-end via an `ImportErrorManager` subclass. |

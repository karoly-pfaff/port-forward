...
### Slices 2–7 — Replay toolkit (delivered)

The replay tool now provides five offline commands — `plan` (Slice 1), `analyze` (Slice 2), `timeline` (Slice 4), `compare` (Slice 5), and `explain` (Slice 6, with optional `--code`/`--list`) — plus a naming/structure polish (Slice 3: binary `replay`, packages `core`/`commands`) and a consistency audit (Slice 7). All commands are strictly offline and read-only, use local tool schemas (`schemaVersion: 1`, never the REST contract), share the `0`/`1`/`2` exit-code convention (no `3`), and support `--json` + `--out` with byte-identical parity. The `tools/replay` module is held to a **≥95% module-wide coverage** bar (currently 96.5%). **Slice 7 (consistency audit): PASS — v1.13 ready for finalization.** The original incident-reconstruction/correlation ideas below remain a possible future direction but are not part of the shipped v1.13 toolkit.

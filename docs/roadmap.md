...
- **Slice 10 (done):** migrated the second volatile read endpoint, `GET /api/config/export` (`server/sources/nest/api/config/`), reusing the Slice-9 clock/provider pattern. Extracted a **shared pure builder** `buildExportedConfig({ rules, now })` into `server/sources/config-export.ts`; the Express `ForwardManager.exportConfig()` builds via it (then emits its `config.exported` activity unchanged) and the Nest `ConfigExportService` builds via the same builder over a narrow `ConfigExportReader`/`CONFIG_EXPORT_READER` + the shared clock — no drift. **Clock reuse:** promoted `ClockReader`/`CLOCK_READER`/`defaultClockReader` to `server/sources/nest/common/clock.reader.ts` (runtime reader re-exports it); `exportConfig(now = new Date())` gained an optional clock arg and the route passes the existing Slice-9 `AppOptions.now` seam, so one production-invisible seam serves both volatile endpoints. **Read-only:** the `config.exported` activity emission (a write side-effect) stays with Express — **no config import/write behavior migrated.** **Exact parity, no normalization:** the parity test boots both apps with the same fixed clock + the same seeded manager so `exportedAt` and rules are deterministic and the booted-Express-vs-booted-Nest diff is byte-for-byte empty (empty + seeded). Response DTO + mapper; no request DTO (no input). Express untouched in behavior, shadow-only, `validate:contract` 234/234, all new/changed code 100% covered, no gate lowered. **`/api/connections` (`generatedAt`) is the last volatile read; config import/apply remain deferred to the config-write milestone.**
- **Slice 11 (done):** migrated the **last** deferred volatile read endpoint, `GET /api/connections` (`server/sources/nest/api/connections/`), reusing the shared clock/provider pattern. Extracted the route's inline per-rule aggregation into a **shared pure builder** `buildLiveConnections({ rules, tcpConnections, udpSessions, now })` in `server/sources/connections-snapshot.ts`; both the Express route and the Nest `ConnectionsService` build via it (no drift) over a narrow `ConnectionsReader`/`CONNECTIONS_READER` + the shared clock. **Exact parity, no normalization, no sockets:** the parity test seeds **fixed** `TcpConnectionInfo`/`UdpSessionInfo` records (never a live registry) into both a seeded Nest fake reader and a small Express `SeededManager`, and pins `generatedAt` with the shared clock, so the booted-Express-vs-booted-Nest diff is byte-for-byte empty (empty + a 1-TCP/1-UDP seeded snapshot) with every field deterministic. Response DTO + mapper; no request DTO (no input). Express untouched in behavior, shadow-only, `validate:contract` 234/234, all new/changed code 100% covered, no gate lowered. **Milestone 2 read-endpoint migration is complete — all GET reads (`ports/advisory`, `activity`, `status`, `forwards`, `runtime`, `config/export`, `connections`) + `DELETE /api/activity` are migrated; the write/lifecycle endpoints (rule CRUD/start/stop/reorder, config import/apply, static serving) remain deferred to Milestone 3+.**
- **Slice 12 (done) — read-migration consistency audit (PASS WITH NOTES):** audited Slices 1–11 for module/provider/token/DTO/builder/error-envelope/parity/coupling consistency before write migration. One small fix (extracted activity's reader/token from `activity.service.ts` into `activity.reader.ts` so all six runtime-backed features keep their reader in a `*.reader.ts` file). Documented intentional notes: `/health` is a scaffold probe outside `/api` (typed constant, no mapper); `ACTIVITY_STORE` is a read+write store token; `GET /api/activity` query coercion is endpoint-local. Added an endpoint inventory table to `server/readme.md`. No new endpoints, no behavior change, no coupling; `validate:contract` 234/234, all touched files 100% covered, gates held. **Also (additional Slice-12 requirement): generated API documentation from NestJS** — added `@nestjs/swagger` + an offline generator (`npm run generate:apidoc`) that builds a tracked, deterministic `docs/api/openapi.json` from Nest controller/DTO metadata (`@ApiTags`/`@ApiOperation`/`@Api*Response`/`@ApiQuery` + decorated `@ApiProperty` schema classes in `nest/common/api-schemas.ts`, each `implements` its shared type). The error envelope (`ApiErrorResponseDto`), all response bodies, `204`/no-body, and query params + 400 are documented; an `openapi.test.ts` drift guard keeps the tracked file current. Schema classes are metadata-only (coverage-excluded); response mappers stay 100% covered. No Swagger UI/runtime change. **DTO rule updated: future endpoint migrations include OpenAPI metadata in the same slice and regenerate `docs/api/openapi.json`.** **v1.14 is ready to proceed to write/lifecycle migration (Milestone 3).**
- **Slice 13 (done) — generated OpenAPI pipeline confirmed:** formally confirmed the generated OpenAPI pipeline (delivered as the additional Slice-12 requirement) meets the full spec, and **hardened the generator tests** to the slice's explicit checklist — an assertion that `GET /api/activity` documents its query params despite endpoint-local coercion, a volatile-endpoint presence check (`/api/runtime`/`/api/config/export`/`/api/connections`), and a clean-app-close test via a minimal injectable app-factory seam. No new endpoints, no runtime change; `openapi.ts` stays 100% covered; `validate:contract` 234/234; the committed `docs/api/openapi.json` is unchanged.
...
- **Slice 14 (done) — `POST /api/forwards` (create):** the first rule write endpoint. Narrow `ForwardRuleCreator`/`FORWARD_RULE_CREATOR` (domain `ForwardManager.addRule`); `201` + `ForwardRuleResponse`; body validation **delegated to the shared `validateForwardRule`** (the `CreateForwardRuleBodyDto` is doc/typing only — no validation pipe, a documented parity exception); manager `ValidationError`/`ConflictError` → `400`/`409` via the shared `mapManagerError` (+ `ApiConflictException`). Byte-for-byte parity for valid TCP/UDP create, `400` validation cases, `409` duplicate binding, and GET-after-create; `enabled:true` starts the forwarder like Express (tests use `enabled:false`, no sockets). OpenAPI documents the POST; `docs/api/openapi.json` regenerated. Express's `POST /api/forwards` unchanged + shadow-only; `validate:contract` 234/234; all new code 100% covered.
- **Slice 15 (done) — `PATCH /api/forwards/:id` (update):** narrow `ForwardRuleUpdater`/`FORWARD_RULE_UPDATER` (domain `ForwardManager.updateRule`); `200` + `ForwardRuleResponse`; partial body validation **delegated to the shared `validateForwardRulePatch`** (`UpdateForwardRuleBodyDto` is doc/typing only; `:id` path param is an unvalidated string documented via `@ApiParam` — both documented parity exceptions); added `ApiNotFoundException` + a `NotFoundError`→404 branch to `mapManagerError` (Validation→400/NotFound→404/Conflict→409). Preserves Express exactly: partial merge never overwrites unspecified fields with `undefined`, and a *running* rule restarts only when a forwarding field changes (metadata-only updates don't restart; stopped rules aren't started — tests use stopped rules, no sockets). Byte-for-byte parity for metadata/forwarding/not-found/invalid-port/invalid-protocol/duplicate-binding/extra-field/GET-after-PATCH. OpenAPI documents the PATCH; doc regenerated. Express's `PATCH /api/forwards/:id` unchanged + shadow-only; `validate:contract` 234/234; all new code 100% covered.
- **Slice 16 (done) — `DELETE /api/forwards/:id` (delete):** completes the rule CRUD trio. Narrow `ForwardRuleDeleter`/`FORWARD_RULE_DELETER` (domain `ForwardManager.deleteRule`); returns **`204` with no body** (matching Express; `@HttpCode(204)` + `void` return). The `:id` path param is an unvalidated string documented via `@ApiParam` (no request DTO, no response DTO — the `204` empty body is the response, the same exception as `DELETE /api/activity`). Not-found maps through the existing shared `mapManagerError` (`NotFoundError`→`404`) — no new error type. **Runtime cleanup** (a running forwarder is stopped first), **persistence rollback**, and the `rule.deleted` emission are all inherited by delegating to `manager.deleteRule`. Byte-for-byte parity for delete-stopped-TCP (`204` + removed from GET), delete-stopped-UDP, unknown id (`404`), and repeated delete (`204` then `404`). OpenAPI documents the DELETE (path param, `204`, `404`); doc regenerated. Express's `DELETE /api/forwards/:id` unchanged + shadow-only; `validate:contract` 234/234; all new code 100% covered.
- **Slice 17 (done) — `POST /api/forwards/:id/start` (lifecycle start):** the first lifecycle endpoint. Narrow `ForwardRuleStarter`/`FORWARD_RULE_STARTER` (domain `ForwardManager.startRule`); returns **`200`** + the rule's `ForwardStatus` (`@HttpCode(200)` is required — Nest defaults `POST`→`201`). The response body is a status, not a rule, so it reuses `ForwardStatusDto` (shared with `GET /api/status`) via a new `toForwardStatusResponseDto` mapper. No request body; `:id` is an unvalidated string documented via `@ApiParam`. Idempotent already-running behaviour, listener open, `rule.started` emission, and not-found (`404`) are inherited by delegating to `manager.startRule`; unknown id → existing `mapManagerError` `NotFoundError`→`404` (no new error type); a start failure → generic `500` (documented Slice-3 divergence, unit-tested, not cross-runtime asserted). **Socket-safe parity:** a started status carries a volatile `startedAt`, so byte-for-byte parity uses the **idempotent already-running** path on a **shared** manager (listener opened once via the Test-A `startRuleStable` bind-retry helper → both runtimes return the identical pinned status; `GET /api/status` after start matches too), plus two single-runtime real cold-start integration tests (TCP + UDP) that open a real listener through the Nest endpoint and `stopAll()` in `finally` — no leaked sockets, no fixed ports. OpenAPI documents the start (path param/`200`/`404`); doc regenerated. Express's `POST /api/forwards/:id/start` unchanged + shadow-only; `validate:contract` 234/234; all new code 100% covered.
- **Slice 18 (done) — `POST /api/forwards/:id/stop` (lifecycle stop):** the natural pair to start, completing the single-rule start/stop lifecycle. Narrow `ForwardRuleStopper`/`FORWARD_RULE_STOPPER` (domain `ForwardManager.stopRule`); returns **`200`** + the rule's `ForwardStatus` (`@HttpCode(200)`), reusing the Slice-17 `ForwardStatusDto` response DTO + mapper unchanged. No request body; `:id` is an unvalidated string documented via `@ApiParam`. Idempotent already-stopped behaviour, listener close, `rule.stopped` emission, and not-found (`404`) are inherited by delegating to `manager.stopRule`; unknown id → existing `mapManagerError` `NotFoundError`→`404` (no new error type). **Simpler parity than start:** a stopped status is fully deterministic (`running:false`, no `startedAt`), so the already-stopped path is parity-tested with **zero sockets** (separate managers), and the running-rule stop is parity-tested by starting each manager on its own free port (Test-A `startRuleStable`) and stopping via the endpoint (+`GET /api/status`-after-stop parity, `stopAll()` cleanup); a single-runtime UDP start→stop test proves UDP first-class. OpenAPI documents the stop (path param/`200`/`404`); doc regenerated. Express's `POST /api/forwards/:id/stop` unchanged + shadow-only; `validate:contract` 234/234; all new code 100% covered.
- **Slice 19 (done) — `POST /api/forwards/reorder` (reorder):** the FIRST migrated endpoint with a **real validated request body DTO**. Narrow `ForwardRulesReorderer`/`FORWARD_RULES_REORDERER` (domain `ForwardManager.reorderRules` + `listRules`); returns **`200`** + the full reordered `ForwardRuleResponse[]` (reusing `ForwardsListResponseDto` + `toForwardRuleResponse`), `@HttpCode(200)`. Unlike create/update (validators delegated to the manager), reorder's `{ ids: string[] }` check is simple, so `ReorderForwardRulesBodyDto` (`@IsArray` + `@IsString({each:true})`, exact Express message) runs through `ApiValidationPipe` — non-array/missing/non-string-element → `400 ["ids must be an array of strings."]`, byte-identical to Express; empty array valid (no-op); whitelist strips extras. Reorder is metadata-only (no socket churn): listed ids first, unlisted rules appended (partial allowed, duplicates tolerated), unknown id → `404` (via `mapManagerError`) with no persist, persist failure rolls back. Socket-free byte-for-byte parity with separate seeded managers (full/partial/duplicate/empty/unknown-404/invalid-400 + GET-after-reorder). OpenAPI documents the reorder (body/`200` array/`400`/`404`); doc regenerated. Express's `POST /api/forwards/reorder` unchanged + shadow-only; `validate:contract` 234/234; all new code 100% covered.
- **Slice 20 (done) — `POST /api/forwards/:id/diagnose` (diagnose):** a read-only diagnostic returning ordered checks + a summary. Narrow `DiagnosticReader`/`DIAGNOSTIC_READER` (`getRule` + `getStatus`; domain `ForwardManager` satisfies it) + the shared `CLOCK_READER`; the service looks up the rule (unknown id → `404` via a direct `ApiNotFoundException`, matching Express's inline check), reads `running`, and runs the **shared `diagnoseRule`** (same checks/order/messages/summary). Returns `200` + `RuleDiagnosticsResult` (new `RuleDiagnosticsResultDto`/`DiagnosticCheckDto`/`DiagnosticSummaryDto` schema classes + a deep-copy `toRuleDiagnosticsResponseDto` mapper); `@HttpCode(200)`; no request body; `:id` via `@ApiParam`. The volatile `diagnosedAt` is pinned via a behavior-preserving `diagnoseRule(rule, isRunning, now = new Date())` seam (Express threads its `AppOptions.now`, Nest passes `CLOCK_READER.now()` — mirroring the `exportConfig(now)` seam, production unchanged). Network-aware byte-for-byte parity uses a **UDP** rule (target-connect always `skip`), a **shared** manager, **sequential** calls (no listen-bind overlap), and a pinned clock — no normalization; the `404` path uses separate managers. OpenAPI documents the diagnose (path param/`200` `RuleDiagnosticsResultDto`/`404`); doc regenerated. Express's `POST /api/forwards/:id/diagnose` unchanged + shadow-only; `validate:contract` 234/234; all new Nest code 100% covered (`diagnose.ts` stays at its pre-existing 98.2%).
- **Slice 21 (done) — `POST /api/forwards/groups/:group/stop` (group stop):** the FIRST migrated group action (of two — start/stop; group **start** opens sockets and stays deferred). Narrow `ForwardGroupStopper`/`FORWARD_GROUP_STOPPER` (domain `ForwardManager.stopGroup`); the service mirrors Express's `handleGroupAction("stop")` — normalizes the `:group` (`decodeURIComponent(...).trim()`), validates with the SHARED `validateGroupName` (→ `400`, delegated — not re-expressed), stops the group, maps empty → `404`, summarizes with the SHARED `summarizeGroupAction`. Returns `200` + `GroupActionResponse` (new `GroupActionResponseDto`/`GroupActionResultDto` + deep-copy `toGroupActionResponseDto` mapper); `@HttpCode(200)`; `:group` via `@ApiParam`. Behaviour over rule metadata (no rule mutation); the service throws the shared `ApiBadRequestException`/`ApiNotFoundException` directly (matching Express's inline `400`/`404`). `GroupActionResponse` has no volatile field, so no clock seam. Stopping a group of `enabled:false` rules is socket-free, so byte-for-byte parity uses **separate seeded managers** (success → all `not_running` skips + GET-unchanged, `404` unknown group, `400` empty group, encoded-group normalization). OpenAPI documents the group stop (path param/`200`/`400`/`404`; group start NOT in the Nest doc); doc regenerated. Express's group stop unchanged + shadow-only; `validate:contract` 234/234; all new Nest code 100% covered.
- **Slice 22 (done) — `POST /api/forwards/groups/:group/start` (group start):** completes the group-action pair (and the whole `/api/forwards` surface). Narrow `ForwardGroupStarter`/`FORWARD_GROUP_STARTER` (domain `ForwardManager.startGroup`); the service mirrors `handleGroupAction("start")` (the Slice-21 stop counterpart) — same normalization (`decodeURIComponent(...).trim()`), shared `validateGroupName` (→`400`, delegated), `startGroup`, empty → `404`, shared `summarizeGroupAction`. Returns `200` + `GroupActionResponse`; **REUSES** the Slice-21 `GroupActionResponseDto`/`GroupActionResultDto` + `toGroupActionResponseDto` (no new DTO); `@HttpCode(200)`; `:group` via `@ApiParam`; throws the shared `ApiBadRequestException`/`ApiNotFoundException` directly. `GroupActionResponse` has NO volatile field, so the challenge is only sockets: `400`/`404`/encoded-normalization are socket-free (separate managers), success uses the **idempotent already-running** path on a **shared** manager (open the listener once via Test-A `startRuleStable`, both runtimes → `already_running` skips, `stopAll()` cleanup), and a single-runtime cold-start test starts a real listener through the Nest endpoint (`started` + GET-status-running) with `stopAll()` cleanup. OpenAPI documents the group start (path param/`200`/`400`/`404`); group stop docs intact; doc regenerated. Express's group start unchanged + shadow-only; `validate:contract` 234/234; all new Nest code 100% covered. **Milestone 3 (write/lifecycle) complete — the entire `/api/forwards` surface is migrated.**
...
- **Slice 23 (done) — `POST /api/config/plan` (non-mutating config dry-run):** the first config-milestone endpoint and the safest (the inventory found `export` (done), `import`/`apply` (mutating), and `plan` (non-mutating)). Narrow `ConfigPlanReader`/`CONFIG_PLAN_READER` (`listRules()`; domain `ForwardManager`) + the shared `CLOCK_READER`; runs the **shared `buildConfigPlan`** (the same pure engine Express calls) and returns `200` + `ConfigPlanResponse` (new `ConfigPlan*Dto` schema classes + a `structuredClone` deep-copy `toConfigPlanResponseDto` mapper); `@HttpCode(200)`. **Non-mutating** — never mutates rules/opens sockets/emits activity. The body check is **delegated** to the service to match Express's `"desired" in body` key-presence semantics (`400` only when the `desired` key is absent; `desired: null` → plan error, not `400` — so `@IsDefined` would diverge; `ConfigPlanBodyDto` is doc-only). The volatile `generatedAt` is pinned via the existing `now` seam — `buildConfigPlan` already had a `now` arg; the Express route now threads `options.now` (behavior-preserving), Nest passes `CLOCK_READER.now()`. Byte-for-byte parity uses a **shared** manager (plan is non-mutating) + the pinned clock (`400`-missing-desired, empty plan, add/update/unchanged drift with a `listRules()` no-mutation assertion, invalid-rule → plan error, `desired: null` → plan error). OpenAPI documents the plan (body/`200`/`400`); config-export docs intact; doc regenerated. Express's `POST /api/config/plan` unchanged + shadow-only; `validate:contract` 234/234; all new Nest code 100% covered.
- **Slice 24 (done) — `POST /api/config/import` (mutating config import):** the first MUTATING config endpoint. Narrow `ConfigImporter`/`CONFIG_IMPORTER` (`importConfig` + `listRules`; domain `ForwardManager`); reuses the SHARED `ForwardManager.importConfig` (replace/merge mutation, duplicate-binding/merge-conflict rejection, persist rollback, enabled-rule start, activity). Validation **delegated** to inline service checks matching Express's SHORT-CIRCUIT order (`400` mode-first then config; class-validator would accumulate both and diverge), so `ConfigImportBodyDto` is doc-only. The **`200 {result, rules}`** and **`422 {errors, result}`** (errors WITH result — NOT the plain `{errors}` envelope, so it must NOT flow through the error filter, which strips extra fields) are **RETURNED with their status via `@Res({ passthrough: true })`** (matching Express's `response.status().json()`); only the two `400`s are thrown as `ApiBadRequestException`. New `ConfigImportResponseDto`/`ConfigImportErrorResponseDto`/`ImportResultDto`/`ConfigImportBodyDto` + `structuredClone` mappers. No volatile field → no clock seam; all `422` paths reject before mutating. Byte-for-byte parity uses **separate seeded managers** (import mutates) with fixed-id `enabled:false` rules (deterministic, socket-free): `400` mode/config, `200` replace into empty + over seeded (+ `GET /api/forwards`/`export` state-after parity), `422` invalid rule (+ no-mutation assertion), `422` duplicate binding. Rollback inherited from `importConfig` (Test-D) + unit-covered via a throwing fake (→ `500`). OpenAPI documents the import (body/`200`/`422`/`400`); export+plan docs intact; doc regenerated. Express's `POST /api/config/import` unchanged + shadow-only (no Express source modified); `validate:contract` 234/234; all new Nest code 100% covered. **Remaining Milestone 4: the mutating `POST /api/config/apply` (volatile `appliedAt` + dry-run/destructive-gating/rollback).**
- **Slice 25 (done) — `POST /api/config/apply` (mutating config apply):** the FINAL config endpoint — completes config export/plan/import/apply. Narrow `ConfigApplier`/`CONFIG_APPLIER` (`listRules` + `importConfig`; domain `ForwardManager`; a separate token from `CONFIG_IMPORTER`); runs the SHARED `buildConfigPlan` + `buildApplyImportFromPlan`, and on the non-dry-run drift path the SHARED `ForwardManager.importConfig` (`replace`) — inheriting rollback/start/activity. **Exact Express ordering:** missing `desired` → `400`; `hasErrors` → `200 ok:false`; `dryRun` → `200 ok:true` (BEFORE the destructive gate — a destructive dry-run is `ok:true`, no `yes`); `destructive && !yes` → `400`; `hasDrift` → replace import (+ belt-and-suspenders import-error guard → `200 ok:false` via `plan.errors`); else → `200 ok:true`. **All success/`ok:false` are `200`** (only the two gating errors `400`), so `@HttpCode(200)` + the service THROWS `ApiBadRequestException` for the `400`s and RETURNS the `ConfigApplyResponse` otherwise — **no `@Res`** (unlike import's `422`). Validation **delegated** to inline service checks (`"desired" in body` key-presence — `desired: null` → plan error, not `400`; + the destructive gate), so `ConfigApplyBodyDto` is doc-only. **Two volatile timestamps** (`appliedAt` + embedded `plan.generatedAt`) pinned from ONE clock instant: Express threads `options.now` into BOTH `appliedAt` and `buildConfigPlan`'s `now` (production-invisible refinement — same instant for both), Nest injects `CLOCK_READER` — **no field stripped/normalized**. New `ConfigApplyResponseDto`/`ConfigAppliedCountsDto`/`ConfigApplyBodyDto` + `structuredClone` mapper. Byte-for-byte parity uses **separate seeded managers** (apply mutates) + fixed-id `enabled:false` rules + pinned clock: `400` missing desired, dry-run (pinned `appliedAt`+`generatedAt`, no mutation), apply-add (+ `GET /api/forwards`/`export` state-after parity), destructive blocked without `yes` (state unchanged), destructive applied with `yes:true` (removed), invalid rule → `ok:false` (no mutation), no-drift → `ok:true`. The import-error guard + persist re-throw (`500`) are service unit-covered (fake applier). OpenAPI documents the apply (body/`200`/`400`); export/plan/import docs intact; doc regenerated. Express's `POST /api/config/apply` behavior-unchanged (only the production-invisible clock seam) + shadow-only; `validate:contract` 234/234; all new Nest code 100% covered. **Config milestone complete; remaining v1.14: static client serving.**
- **Slice 26 (done) — static client serving (SPA fallback):** migrated static serving into the Nest shadow runtime (`server/sources/nest/static/static-serving.ts`), completing the v1.14 endpoint/serving migration — only the final audit/switch-readiness review remains. **Inspected Express:** gated on `staticClientDir && hasStaticClient(dir)` (usable only with `index.html`); `express.static(dir)` for assets + a catch-all serving `index.html` for non-`/api` paths (SPA); a missing dir is allowed (API + `/api/*` JSON-404 stay usable). **Nest:** `resolveStaticOptions`/`hasStaticClient` (mirror the enable rule), `configureStaticAssets(app, dir)` (`app.useStaticAssets` = the `express.static` equivalent), and a `StaticFallback`/`STATIC_FALLBACK` strategy that serves `index.html` for an unmatched non-API GET/HEAD route. Since Nest surfaces unmatched routes as `NotFoundException` → the global `ApiErrorEnvelopeFilter`, that filter (the single unmatched-route handler) now ALSO owns the non-API SPA fallback (delegating to the injected `StaticFallback`); the `/api/*` envelope branch runs first, untouched. **Shadow-only & API-safe:** `STATIC_FALLBACK` defaults to `disabledStaticFallback` (scaffold wires no dir), so live behavior is unchanged + the API works with no client build; proven via tests + the reusable helper, with live `start:nest` wiring deferred to the switch slice (no `main.ts`/`bootstrap.ts` change). **Express↔Nest parity** (raw status+body): `/`, SPA routes, real asset, missing asset → index.html, unmatched `/api/*` → JSON envelope — all identical; plus disabled-static (API usable, no SPA; non-API 404 *body shape* differs — Express HTML vs Nest JSON — documented boundary). **OpenAPI:** static adds NO routes (`openapi.json` unchanged; a generator test asserts only `/health` + `/api/*` paths). Express remains the default runtime; Nest static is shadow-only; `validate:contract` 234/234; all new Nest code 100% covered. **v1.14 ready for the final migration audit / switch-readiness review.**

- **Slice 27 (done) — OpenAPI artifact placement & DTO/schema hygiene cleanup:** a cleanup/refactor before the final audit; no endpoint behavior change, no runtime switch. **OpenAPI placement:** `generate:apidoc` writes a primary server-owned `server/build/api/openapi.json` (gitignored) + a synced tracked `docs/api/openapi.json` copy; pure helpers `resolveOpenApiPaths`/`writeOpenApiArtifacts`/`copyOpenApiToRelease` (packaging copies the primary artifact into a release dir without regenerating); deterministic across transforms via sorted `components.schemas`. **Schema ownership:** the centralized `common/api-schemas.ts` (37 classes) dissolved into feature-local `*.schema.ts` files; `common/` keeps only `ApiErrorResponseDto`; class names unchanged → OpenAPI semantically identical (sorted ordering + reworded description only); coverage exclusion → `sources/nest/**/*.schema.ts` glob (mappers stay covered). **Comment cleanup:** slice/migration-history language removed from NestJS code comments. **Legacy guideline (docs only):** the Express server will later move under `legacy/` for a rollback path — future work, no move now, Express stays active. `validate:contract` 234/234; all new helpers 100% covered, no gate lowered. **v1.14 ready for the final migration audit / switch-readiness review.**
...
- **Slice 30 (done) — server structure cleanup, 100% coverage gate, release OpenAPI artifact:** the post-switch hardening slice — NestJS becomes the **normal server**, not a migration scaffold. **Legacy Express removed** (`sources/legacy/`, the parity harness, and the 11 Express↔Nest `*.integration.test.ts` parity tests deleted; reusable domain modules stay; `cors` dropped, `express` kept via `@nestjs/platform-express`). **`sources/nest/` flattened** into a clean tree `sources/{app, api/<feature>, common, health, openapi, static}/` + framework-free domain modules. **Coverage gate raised to 100/100/100** honestly — direct unit tests re-cover the manager behaviors the deleted integration tests drove, and the few structurally-unreachable defensive branches (2 s diagnose socket timeouts, post-stop socket races, nullish-on-initialized-counter fallbacks) are narrowly `/* v8 ignore … */`-annotated (no broad exclusions). **OpenAPI release artifact wired** (`copy:apidoc:release` → `copyOpenApiToRelease` copies the primary `server/build/api/openapi.json` into `<package>/api/openapi.json`; the platform build scripts run it; `validate-runtime` asserts it). **Packaged `server.js` now bundles NestJS** (esbuild externals + a `{ "type": "commonjs" }` package-dir marker fixing a pre-existing CJS-as-ESM crash; boot-verified); Go remains the preferred packaged runtime. Legacy/shadow/scaffold wording removed from `server/readme.md` + comments; `dev:legacy`/`start:legacy`/`start:nest`/`test:nest` scripts removed. `validate:contract` **234/234**; `validate:coverage:server` 100/100/100; `validate:runtime:smoke` 24/24; 616 server tests; lint/typecheck/build green; full `npm run test` green. **v1.14 is complete — NestJS is the single TypeScript server; ready for the final v1.14 release audit/tag.**

- **Slice 31 (done) — server source layout cleanup and NestJS logging consolidation:** structural cleanup before the final v1.14 release audit. No API behavior/contract change; `validate:contract` **234/234**; `docs/api/openapi.json` unchanged. **Root `sources/` now holds only `index.ts`** — domain/runtime/persistence files moved into homed concern folders: `forwarders/forward-manager.ts`, `config/{config-export,config-plan}.ts`, `persistence/config-store.ts`, `connections/connections-snapshot.ts`, `runtime/runtime-info.ts`, `diagnostics/diagnose.ts`, `app/server-options.ts`, `testing/test-helpers.ts` (a path-aware codemod rewrote 58 import specifiers; `git mv` preserved history). **Root `common/` dissolved** — its files are API-layer-only, so the shared API infra (error envelope/filter/exceptions, validation pipe, shared `CLOCK_READER`, the one cross-feature `ApiErrorResponseDto`) moved to **`api/common/`**; `runtime-context.ts` → `app/` and `manager-error.ts` → `api/forwards/` left separately. **Logging consolidated onto NestJS `Logger`** — removed the bespoke structured-console logger; the entry uses `new Logger("Server")` and `createNestApp` enables/silences the NestJS logger by runtime (`resolveLoggerOption`); the only remaining `console.*` are the logic-free OpenAPI build-tooling entries. DTO/schema ownership stays feature-local; no `sources/nest/`/`sources/legacy/` returns. Coverage gate stays **100/100/100**; `validate:runtime:smoke` 24/24 (packaged `server.js` boots with NestJS logging); 614 server tests; lint/typecheck/build green; full `npm run test` green. **v1.14 server tree is final-audit-ready.**

- **Slice 32 (done) — final v1.14 release audit & readiness review — READY WITH NOTES:** audit + small remediation only; no feature/API/behavior change; `validate:contract` **234/234**. Verified the runtime story (one NestJS TypeScript server; packaged `server.js` bundles NestJS and boots; Go preferred packaged runtime; no Express runtime/scripts/imports; no `sources/nest|legacy|common`), the final source layout (root `sources/`=`index.ts`; API + `api/common/` under `api/`; domain/runtime/persistence homed), NestJS-`Logger` runtime logging, the **100/100/100** gate, the OpenAPI primary/docs/release-copy flow, static + `/api/*` envelope, and runtime smoke. **Remediation:** reworded ~80 stale "Express"/"the Express route|manager"/"cannot drift"/"are migrated/stay with Express" references in production comments + ~13 in test descriptions to current `/api`-contract wording (no behavior change) — the generated `docs/api/openapi.json` now has **0 "Express"** in operation descriptions (was 9 user-facing); fixed `docs/architecture.md`, `docs/api-contract.md`, and `docs/glossary.md` current-state runtime references (`api.ts`/Express → NestJS); historical slice-logs keep period-accurate wording. **Version NOTE:** versions remain **1.13.0** and the changelog header is `[Unreleased] — v1.14 (in progress)` — the bump to **1.14.0** + changelog finalization + tag are the deliberate release step, not this audit. lint/typecheck/build clean; full `npm run test` green; `validate:coverage:server` 100/100/100; `validate:runtime:smoke` 24/24 (OpenAPI 0-Express + Go smoke + static UI); packaged `server.js` boot-verified; replay skipped (no replay/shared files). **v1.14 can be tagged next.**
...
**Status:** COMPLETE — finalized as **v1.15.0**, ready to tag (tag not yet created). Slices 1–14 done (Go router migration: chi owns every route, no legacy dispatch, no migration scaffolding, `app.App` is the single dependency container; Slice 14 cleaned the migration-era comments/wording). Finalization bumped all version surfaces to 1.15.0, finalized the changelog, and ran the full validation matrix green (`validate:contract` 234/234, `validate:openapi:go` green, `validate:coverage:service` PASS at 90/95, `validate:runtime:smoke` 24/24, lint/typecheck/`npm run test` green). Slice 1 (router structure audit & modular target layout) recorded the as-built inventory, endpoint inventory, coupling audit, target layout, route-registration pattern, error/response ownership plan, 8-slice plan, and a router-dispatch characterization test (`service/sources/api/router_contract_test.go`) in `audits/v1.15-go-router-audit-1.md`. Slice 2 introduced the explicit dependency container `app.App` (`service/sources/app/`) + `app.New(...)`, made `api.NewHandler(app)` the construction path (removed `api.Options`), added the `modularRoute`/`dispatchModular` route-registration skeleton (`api/routes.go` + per-feature `health_routes.go`/`runtime_routes.go`), and migrated `GET /api/health` + `GET /api/runtime` into it. Slice 3 extracted the shared HTTP request/response/error helpers (`writeJSON`/`decodeRequest`/`readBody`/`writeManagerError`) out of `api.go` into `api/respond.go` (behavior-preserving). Slice 4 added a read-only Go API inventory drift gate (`npm run validate:openapi:go`) that compares the Go service's declared route/method/status surface against the canonical `server/build/api/openapi.json` (the NestJS-generated artifact stays the single source of truth; `docs/api/openapi.json` is not the comparison target), with explicit whitelists for the Go-only additive `GET /api/health` and the OpenAPI-only `GET /health`; it complements `validate:contract`, doesn't replace it. Slice 5 migrated the first non-health/runtime feature handler — `GET /api/ports/advisory` — out of the ordered `serveAPI` dispatch into a feature route module (`api/ports_routes.go`). Slice 6 migrated the activity routes (`GET`/`DELETE /api/activity`) into `api/activity_routes.go`, introducing the skeleton's first multi-method same-path registration and its first 204/no-body modular response. Slice 7 migrated `GET /api/status` into `api/status_routes.go`. Slice 8 migrated `GET /api/connections` into `api/connections_routes.go`, moving its connections-owned helpers (`handleConnections` + `buildRuleLiveSummary`) out of `api.go` (behavior-preserving, double-guarded by `validate:contract` + `validate:openapi:go`). Slice 9 migrated the forward-rule read/list `GET /api/forwards` into `api/forwards_routes.go` (only the exact read route — `POST`/reorder/`:id`/group/lifecycle prefix routes still fall through to the legacy dispatch; the shared `rulesToResponses`/`toRuleResponse` mappers stay in `api.go` until the forwards write group migrates). Slice 10 introduced **`github.com/go-chi/chi/v5` as the Go API router foundation** (the project decision — the prior `net/http` skeleton was intermediate): a chi router (`buildAPIRouter`) owns the 8 migrated routes; its `NotFound`/`MethodNotAllowed` both delegate to the legacy ordered `serveLegacyAPI` dispatch so unmigrated routes keep working and the generic `/api` 404 envelope (404-not-405) is preserved; the top-level API/static boundary stays owned by `Handler.ServeHTTP` (chi only serves `/api`). Slice 11 migrated the **entire forwards write/ID/lifecycle/group surface** onto chi `{id}`/`{group}` patterns (`POST /api/forwards`, `PATCH`/`DELETE /api/forwards/{id}`, `POST /api/forwards/{id}/{start,stop,diagnose}`, `POST /api/forwards/reorder`, `POST /api/forwards/groups/{group}/{start,stop}`) — the chi patterns identify route shape only, while each handler extracts the id/group with the original logic (ids from `r.URL.Path`, groups from `r.URL.EscapedPath()` + `url.PathUnescape`, never `chi.URLParam`), so encoded `/`/space in group names is preserved (guarded by `TestGroupActionEncodedSpace`/`TestGroupActionEncodedSlash`); static routes win over `{id}` so reorder/group are never misrouted; 404-not-405 preserved. After Slice 11, `serveLegacyAPI` owned **only** the config routes. Slice 12 migrated those config routes (`GET /api/config/export`, `POST /api/config/{import,plan,apply}`) into `api/config_routes.go` and **retired `serveLegacyAPI` entirely** — every API route is now chi-owned, and the former terminal 404 envelope became the small `writeAPINotFound` helper wired to chi `NotFound`/`MethodNotAllowed` (preserving 404-not-405). The top-level API/static boundary stays owned by `Handler.ServeHTTP`. Slice 13 was the final cleanup/audit pass: it **removed the temporary `Handler.manager` bridge** (handlers now read `h.app.Manager` directly — `app.App` is the single dependency container; `Handler` is `{app, staticAvailable, apiRouter}`), confirmed chi owns all 21 routes with no legacy if-chain/`dispatchModular`/`serveLegacyAPI`, added an always-on `TestRouteInventoryMatchesModularRoutes` guard tying live chi registration to the OpenAPI inventory both directions, and cleaned the remaining stale migration-scaffolding comments. No API/contract/packaging/runtime change; `validate:contract` 234/234. **The v1.15 Go router migration is complete** — `api.go` is infrastructure-only. Next: a v1.15 release-readiness audit, or proceed to the next roadmap item.

### Preferred Technical Direction

- **Decided (Slice 10): the Go API router foundation is `chi`** (`github.com/go-chi/chi/v5`) — a lightweight `net/http`-compatible router. The top-level API/static boundary stays owned by Portier (`Handler.ServeHTTP`); chi owns only the migrated `/api` routes, with `NotFound`/`MethodNotAllowed` delegating to the legacy dispatch to preserve 404-not-405 and the unmigrated routes. No Fiber/fasthttp, no DI container.

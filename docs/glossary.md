...
| **Workflow plan** | The deterministic dry-run result of validating a **workflow** (`{schemaVersion, name, steps:[{id, type, status, message, inputs, dependsOn}], summary, result}`; v1.11). Each step is `valid`/`invalid`; the plan `result` is `valid` only when every step is valid. | A validation/plan report, kept **separate** from the policy/doctor report models. Step `inputs`/`dependsOn` describe what a step *would* use; the plan never executes a step or opens the files a step refers to. |
...
- Use **"workflow"** for an ordered sequence of existing safe operations defined in a JSON file (CLI `workflow plan`), and **"workflow plan"** for its dry-run validation result; keep both distinct from "recipe" (a future execution feature). **Workflow step types** (e.g. `policy.check`) are stable operator-facing identifiers. A workflow composes existing operations — it is not a scripting language; v1.11 Slice 1 plans/validates only (no execution).

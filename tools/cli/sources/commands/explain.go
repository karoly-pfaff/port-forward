...
// explanations is the static registry mapping every stable doctor/check code
// (Slices 1 and 2) to its explanation. Keys reuse the doctor code constants so
// the strings cannot drift. Every code the doctors can emit MUST have an entry
// here (guarded by explain_internal_test.go).
var explanations = map[string]Explanation{
	// --- config doctor (Slice 1) ---
...
func printExplanationHuman(exp Explanation, w io.Writer) {

...
func TestRunStart_APIError(t *testing.T) {
	rule := client.ForwardRuleResponse{
		ID: "rule-1", Name: "Dev API", Protocol: "tcp",
		ListenHost: "127.0.0.1", ListenPort: 48000,
		TargetHost: "192.168.1.10", TargetPort: 8080, Enabled: true,
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && strings.HasSuffix(r.URL.Path, "/start"):
			// Non-connection API error (rule already running)
			w.WriteHeader(http.StatusConflict)
			json.NewEncoder(w).Encode(map[string][]string{"errors": {"rule is already running"}})
		default:
			t.Errorf("unexpected %s %s", r.Method, r.URL.Path)
			http.NotFound(w, r)
		}

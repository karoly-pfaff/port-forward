...
if ($UseNode) {
  ...
  if (-not (Test-Path $serverJs)) {
    throw "Node fallback expected server.js at '$serverJs'. Run 'npm run build:runtime:windows' first."
  }
  ...
  if (-not (Test-Path $binary)) {
    throw "Go service binary not found at '$binary'. Run 'npm run build:runtime:windows' first, or use -UseNode for Node.js fallback."
  }

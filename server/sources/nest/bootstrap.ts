...
/**
 * Builds the NestJS scaffold and starts listening on the given options.
 *
...
export async function bootstrap(listen: NestListenOptions): Promise<INestApplication> {
  ...
  console.log(
    `Portier NestJS scaffold listening on http://${listen.host}:${listen.port} ` +
      `(scaffold only — the existing TypeScript server remains the active runtime).`
  );
  return app;
}
...
export function reportBootstrapFailure(error: unknown): void {
  console.error("Portier NestJS scaffold failed to start.", error);
  process.exitCode = 1;
}

...
/**
 * Process entry for `npm run copy:apidoc:release -- <releaseDir>` — copies the
 ...
 * regenerates the document; packaging runs `generate:apidoc` first so the primary
...
if (!releaseDir) {
  console.error("usage: copy-release.ts <releaseDir>");
  process.exit(2);
}

...
/**
 * Transport adapter for the NestJS server liveness probe. Controllers in the NestJS
 * server are HTTP adapters only — all behaviour lives in the service.
 *
 * The dependency is wired with an explicit `@Inject` token so the NestJS server does
...
export class HealthController {
  ...
  @Get("health")
  @ApiOperation({ summary: "Liveness probe", description: "Liveness probe (outside the frozen /api contract)." })
  @ApiOkResponse({

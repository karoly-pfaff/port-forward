 ...
 *
 * Coercion is intentionally contract-faithful so the route stays
 * byte-for-byte parity-identical:
 * - `port` is coerced with `Number(...)` (NaN for missing/non-numeric) and must
 *   be an integer in `[1, 65535]`. All three constraints share the exact contract
 ...
 *   documented `typeof value === "string" ? value : undefined` rule.
 */

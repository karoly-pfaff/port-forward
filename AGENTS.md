...
Client numbers vary ±1% per run (Windows vitest ghost-entry deduplication — both runs pass gates).
...
**Durable rule (v1.10 Slice 2) — policy/doctor explanations:** Every stable doctor/check code AND every policy finding code MUST have an explanation, guarded per-domain (`doctor/explain_internal_test.go`, `policy/explain_internal_test.go`); a new code adds its explanation in the same slice. There is **one** `Explanation` type + **one** set of render helpers in the leaf package `tools/cli/sources/explain` (`For`/`Sorted`/`SortedCodes`/`ForReport`/`PrintInline`/`Merge`, imports nothing internal) — no second schema, no duplicate inline renderer. Each domain OWNS its explanation **data** keyed by its own code constants (`doctor.Explanations()`, `policy.Explanations()`); the `explain` command composes them with `explain.Merge(...)` (dispatch-level composition — explanation data/logic never lives in `commands`). Inline `--explain` (doctor/config doctor/policy check) reuses the registry via `explain.PrintInline`/`explain.ForReport` and must **not change** checks/findings, summary, result, or any exit code — the JSON `explanations` map is additive (`omitempty`, only with `--explain`; output without it byte-identical). Explanations are static offline reference text — they must **never claim Portier enforces/fixes/probes anything**; a policy explanation frames the finding as a **policy choice** (an `allow*`/`require*` guardrail) offering both resolutions (change the rule OR adjust the policy), not necessarily a defect. Doctor `--explain` stays byte-identical; support-bundle `explanations.json` stays doctor-scoped (`doctor.SortedExplanations()`, 20), not the merged 26. Codes sort `config.*` < `policy.*` < `rules.*`/`runtime.*`. New code-emitting domains add their own registry + parity guard + register in the merge — not a central registry framework. CLI stays a pure client; no API/DTO/contract change. (Prior `doctor.Explanation`/`ExplanationFor`/`SortedExplanationCodes` references are superseded — type is in `explain`.)

**Durable rule (v1.10 Slice 3) — policy report export:** Policy report export (`policy check --out`) must write the **same JSON schema as `policy check --json`** (the `reportJSON` `{findings, summary, result}` shape, plus the additive `explanations` map under `--explain`) — no second schema; reuse `output.WritePrettyJSON` (pretty, create/truncate, no parent-dir creation, no partial write on marshal failure). Export must **not mutate** the config/policy file, **not contact the runtime** or probe targets, and add no telemetry. A **file-write failure → exit 1** (`Error writing` on stderr) overriding the report code; a missing `--out` value → exit 2; an unreadable/malformed config or policy still exits 2 and **writes no file** (parse fails before any write — never a success-looking report). `--out` writes regardless of `--json`; in `--json` mode stdout stays pure JSON (no confirmation line) and the **same bytes** go to stdout and the file (build the payload once → `--json --out` and `--json --explain --out` byte-identical); human mode keeps human stdout + a `Report written to <file>` line. Payload construction stays in `policy.Emit`/`EmitOptions.OutPath`, file writing in `output`, `commands` handlers-only. Mirrors the v1.9 doctor `--out` rule — keep the two aligned. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.10 Slice 4) — policy templates:** Built-in policy templates (`policy template <name>` / `--list`) must render to the **exact schema `policy check` accepts** (`schemaVersion: 1` + the five boolean guardrails) so a rendered/`--out` file is directly usable by `policy check` with no editing. Templates are **deterministic, sorted by name** (`Templates()` returns a sorted copy), and live in the `policy` package (`policy/template.go`: registry, metadata, `PolicyValue`/`DetailValue`/`PrintTemplateList`) — **definitions must NOT live in `commands`** (handlers-only: parse flags, resolve template, call `output` for writing). Templates **never** add a protocol allowlist/denylist or `allowUdp` (UDP first-class) and never add new guardrails — starter files only, guarded by a registry test (each parses+evaluates, sorted/unique, no `udp`/`protocol` token, guardrail booleans locked to purpose). The `--out` file is the **bare** policy JSON (no metadata wrapper) regardless of `--json`, via `output.WritePrettyJSON` (no parent-dir creation, no partial write); write failure → exit 1; unknown/missing template, too many names, missing `--out` value, or `--list`+name/`--out` → exit 2. Fully offline; modifies nothing but `--out`. Adding a template = a registry entry + a locked-booleans test case; do NOT grow into a template loader / user template dir / parameterized generator. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.10 Slice 5) — policy-aware config review:** `policy review --current <file> --candidate <file> --policy <file>` must be **dry-run and fully offline** — read the three files, never mutate them, never contact the runtime, never probe targets, no import/apply. Evaluate **only the candidate** against the policy and **reuse the `policy check` finding/report semantics** (`policy.Evaluate` + the shared `printFindings`/`printSummaryAndResult` + the embedded `Report`) — no second finding/report/explanations schema (JSON is `{review, findings, summary, result}` + an additive `explanations` map under `--explain`, exactly like `policy check`). The change summary stays **compact/deterministic** — scalar `current`/`candidate` (`config.BuildSummary`) + a scalar `delta`; **not** a full config diff engine (no per-rule/field diff, rename detection, or diff DSL in this slice). Model/build/render live in `policy/review.go` (`Review`/`ReviewSummary`/`ReviewDelta`/`BuildReview`/`PrintReviewHuman`/`EmitReview`); summaries reuse `config.BuildSummary`; file writing reuses `output.WritePrettyJSON`; `commands` stays handlers-only. Exit: 0 candidate passes, 1 candidate violates OR JSON-encode/`--out` write failure, 2 usage or unreadable/malformed config or policy. `--explain`/`--out`/`--json` behave as in `policy check` (additive explanations never change findings/summary/result/review/exit; `--json --out` byte-identical). CLI stays a pure API client; no API/DTO/contract change, no apply/import/enforcement.

**Durable rule (v1.10 Slice 6) — runtime policy check:** `policy check` takes exactly one config source — `--config` (offline) XOR `--runtime` (live); both/neither → exit 2. Runtime mode is **read-only/dry-run**: read the runtime config ONLY via the existing read-only config-export path (`client.ExportConfig()` / `GET /api/config/export`) — **no new endpoint**, no probing, no enforcement, no mutation. **Reuse the exact offline evaluator + report shape** (`policy.Evaluate` + shared rendering + the same `reportJSON`) — runtime and offline produce identical findings/summary/result for equivalent config; no new policy codes, no second schema. **Offline `--config` stays fully offline** — never resolve/contact the runtime URL (lazily resolve `commands.ConnFlags` only in the `--runtime` branch); the `policy` package stays free of live-client behavior (the command owns the fetch via `loadCheckRules`/`exportedRules` + lazy client; `commands` handlers-only). `--policy` is validated BEFORE any runtime I/O (input error 2 precedes runtime errors). Runtime-unreachable → exit **3**, config-export/API failure → exit **1** (via `exitWithError`) — never a fabricated policy finding/report, no report invented when the config can't be read. The additive `source` field (`"config file"`/`"runtime"`, `omitempty`) keeps offline `policy check --json` byte-identical when unset and must not leak onto `policy review` (it lives on the policy-check `reportJSON` wrapper, not on `Report`). `--json`/`--out`/`--explain` behave as in offline check. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.12 Slice 5) — workflow history prune:** `workflow history prune` is **destructive and MUST require `--yes`** (missing → exit 2, NO mutation), mirroring `history clear`. It mutates **only the compact local history store** — NEVER the runtime, workflow execution, referenced files, logs/env/process, or configs/policies/reports. **Modes must be explicit and unambiguous**: exactly one of `--keep <n>` (keep newest N, `N >= 0`, `--keep 0` removes all) OR filters (`--result`/`--workflow`/`--code`, AND-combined, which REMOVE matching) — no mode → 2, `--keep`+filters → 2, invalid `--keep`/filter → 2; **`--limit` is NOT accepted**. Validate mode + `--yes` BEFORE reading/writing the store (every usage error leaves it untouched). **Empty match and missing/empty history are successful no-ops** (exit 0, removed 0) — a no-op prune must NOT rewrite or create the file. Prune model/pure logic (`HistoryPruneOptions`/`HistoryPruneResult`/`PruneHistory`) live in the `workflow` package (must NOT import `client`/`commands`); `PruneHistory` stays pure (never mutate input runs); persistence via `HistoryStore.Save`; `commands` handlers-only (parse+validate mode/`--yes`, load, prune, save only when removed>0). `--json` shape `{schemaVersion, removed, kept, totalBefore, totalAfter}` — keep stable. Exit: 0 success (incl. removed 0), 1 read/write failure, 2 usage error; NO 3. No date pruning, no restore/import/sync/telemetry/upload. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.12 Slice 4) — workflow history stats:** `workflow history stats` must operate **only on the compact local history metadata** — NEVER contact the runtime, execute a workflow, read workflow-referenced files, scan raw reports, collect logs/env/process data, or mutate the store. It must **reuse the `workflow history list` filters** (`--result`/`--workflow`/`--code`/`--limit`, via the shared `parseHistoryFilters`), AND-combined, **applied BEFORE computing stats** — no new filter types. **Empty history and empty match sets are successes (exit 0) with zero stats**; an invalid filter value is exit 2; a read failure is exit 1; NO exit 3. **Code counts count RUNS containing a code, not occurrences** (codes are deduped per run) — keep tested. Grouped tables (workflows/step types/codes) are **deterministically sorted (count desc, then key asc)** and always serialize as arrays (`[]` when empty); fixed result/step buckets are objects with stable keys; `filters` appears only when active. Stats model/counting/sorting (`HistoryStats`/`BuildHistoryStats`/`sortCounts`) live in the `workflow` package (must NOT import `client`/`commands`); `commands` stays handlers-only and shares the filter parse/validate (`parseHistoryFilters`), store load (`loadHistoryForRead`), and Filters-block render (`printHistoryFilterBlock`) with `list`. `--json` shape `{schemaVersion, filters?, totalStored, shown, results, steps, workflows[], stepTypes[], codes[]}` — keep stable. No date bucketing/trend charts/raw-report scanning/import/sync/telemetry/upload. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.12 Slice 3) — workflow history filters:** `workflow history list` filters (`--result`/`--workflow`/`--code`/`--limit`) operate **only on the compact local history metadata** — NEVER contact the runtime, execute a workflow, read workflow-referenced files, scan raw reports, or collect logs/env/process data, and NEVER mutate the store. Filters **combine with AND**; `--limit` applies AFTER filtering → the **newest N** matches (store is newest-first). An **empty match set is success (exit 0)**, not an error — including an unknown `--code` (history may contain older non-explainable codes; do NOT require a filter code to be explainable). An **invalid filter value is exit 2**: unknown `--result` (allowlist `passed`/`failed` via `workflow.ValidHistoryResult`), explicitly-empty `--workflow`/`--code`, or non-positive/non-numeric `--limit` (distinguish set-but-empty from absent with `fs.Visit`). Model/matching/list-view (`HistoryFilters`/`ApplyHistoryFilters`/`HistoryListView`/`BuildHistoryListView`/`ValidHistoryResult`) live in the `workflow` package (must NOT import `client`/`commands`); `commands` stays handlers-only (parse+validate flags, call helpers). `--json` list view is `{schemaVersion, filters?, shown, totalStored, runs}` — `filters` present only when active (only active fields, `omitempty`), `runs` always an array. No filtering over raw reports, no history import/sync/telemetry/upload, no new result values beyond what the store holds. CLI stays a pure API client; no API/DTO/contract change (the `list --json` additions are a CLI-only shape change).

**Durable rule (v1.12 Slice 2) — workflow history export:** `workflow history export --out <file>` must be **compact, deterministic, local, offline, read-only** — NEVER mutate/truncate the history store, contact the runtime, execute a workflow, read workflow-referenced files, collect logs/env/process data, or upload. The snapshot carries the **same compact metadata as the store and nothing more** — never raw configs/policies, full reports, host/port payloads, file contents, logs, env vars, process data, secrets, runtime URLs, or tokens; reuse the stored `HistoryRun` entries **verbatim** (newest-first, codes exactly as stored — no re-order/re-dedupe). Include an explicit **`safety` object with all-false flags** (self-documenting exclusion statement; guard with a test). Empty/missing history → a valid **`runCount: 0`** snapshot (exit 0), never an error. Export model/projection (`BuildHistoryExport`, injected clock) live in the `workflow` package (`workflow/history.go`); `commands` stays handlers-only (parse flags, load store, call `BuildHistoryExport`, write via `output`); `workflow` history code must NOT import `client`/`commands`. Build the payload ONCE so `--json` stdout and the `--out` file are **byte-identical** (test it). Exit: 0 written (incl. empty); 1 history read / output write / JSON-encode failure; 2 missing `--out` value. `--out` parent dir must already exist (match `policy check`/`doctor`/`workflow template` `--out` — no MkdirAll); existing file overwritten. No filtering/import/sync/telemetry/upload/AI-handoff for history (AI handoff is the `workflow report` bundle). CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.12 Slice 1) — local workflow run history:** Workflow run history is **OPT-IN and local-only** — `workflow run` records NOTHING unless `--record-history` is passed; no always-on/background collection, no scheduler, no telemetry, no upload/sync. Store **compact metadata only** — run id, time, workflow name, result, summary counts, compact per-step metadata (`id`/`type`/`status`/`exitCode`), and the deduped+sorted emitted explanation/finding codes; **NEVER** raw configs/policies, full embedded policy reports, file contents, logs, env vars, process data, secrets, runtime URLs, or tokens (drop each step's embedded report + message). History is **bounded** (latest 100, newest first) and must **not alter workflow execution semantics** except history-write-failure handling: a write failure is a WARNING that never hides the run result (passed run → exit 1; an already-failed run keeps 1/3, runtime-unreachable stays 3), and an **invalid plan is never recorded** (completed runs only). Model/projection (`ProjectRun`)/run-id (`NewRunID`)/store (`HistoryStore`: load/append+retention/clear) live in the `workflow` package (`workflow/history.go`) and must NOT import `client`/`commands` or contact the runtime; `commands` stays handlers-only (`workflow_history.go` — flag parsing, path resolution via an injectable seam, NOT `os.Getenv` in a workflow handler, rendering). `workflow history list`/`show`/`clear` are fully offline; exit: 0 success (incl. empty list / already-empty clear), 1 unknown id (`show`) or read/write failure, 2 missing run id (`show`) or `clear` without `--yes`. A new stored field must keep the compact/no-sensitive-data guarantee (no-banned-content test). Do NOT add background recording, a daemon, a UI history panel, remote storage, or AI handoff for history (AI handoff is the `workflow report` bundle). CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 7) — workflow support report:** `workflow report --from <report.json> --out <dir>` is a **packaging step over an EXISTING report only** — it must NEVER execute a workflow, run a shell command, contact the runtime, read the config/policy/baseline/report files a step refers to, collect logs/env/process data, mutate the input, or upload. It parses ONLY the provided `workflow plan`/`run` `--json` report and re-derives explanations from the **canonical `explain` registry** (workflow step + run + `policy`, via `reportExplainRegistry`) — no second schema, no fresh evaluation. The bundle carries **no secrets/raw config/env/process/log data**: `report.json` is a NORMALIZED view (step id/type/status/message + per-step explainable codes) that DROPS each run step's embedded report; `manifest.json`/`summary.txt`/`explanations.json` (`{}` when none) carry metadata + the step list + code explanations; `summary.txt` keeps a fixed safety block. Codes (deduped, unknown dropped): invalid plan step → `workflow.step.*`; skipped run step → `workflow.run.dependency_failed`; failed run step → embedded `policy.*` (`findings`/`new`); plus the input's `explanations` map keys. Output only inside `--out`, following the `support-bundle` convention (create if missing, **refuse non-empty**, never delete unrelated files). Exit: 0 written; 1 create/write or `--json` encode failure; 2 usage or unreadable/malformed/unsupported report; **no 3**. Model/parser/extractor/summary/writer in the `workflow` package (`workflow/report.go`); `commands` handlers-only (`RunWorkflowReport` reuses `prepareBundleDir`); `workflow` must NOT import `client`/`commands`. Detection = `schemaVersion` + result label + `steps` present (plan AND run supported). AI prompt `prompts/workflow.md` is docs-only (no `--prompt`, no AI/upload/telemetry; warns against secrets; forbids inventing blind config changes). CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 6) — workflow run explanations:** `workflow run --explain` is **presentation only** — it must NEVER change which steps run, their status, the run summary/result, or any **exit code** (including runtime-unreachable's exit 3); guard with a decode-and-compare test (run with/without `--explain` → identical steps/summary/result). **Passed steps are never explained.** Explanations are **additive**: human adds an inline `explain.PrintInline` block under each FAILED/SKIPPED step; JSON adds a top-level `explanations` map populated ONLY with `--explain`, ONLY for emitted codes (deduped), `omitempty` so without `--explain` (and when no explainable codes were emitted) the JSON is byte-identical to before, and `--json --out --explain` stdout/file stay byte-identical. **Reuse the canonical `explain` registry/renderer** — one `Explanation` type, `explain.PrintInline`/`explain.ForReport`; no second schema, no duplicated policy explanations. A failed policy step is explained by its **policy finding codes** (from its report), NOT a fabricated run code; the `workflow.run.*` codes are ONLY for run-specific outcomes — `workflow.run.dependency_failed` (reportFrom dependency produced no report → skip), `workflow.run.runtime_unreachable`, `workflow.run.input_failed` (unreadable/malformed referenced file or runtime config). Run codes are **stable operator-facing identifiers** — exactly 3, locked by `run_explain_internal_test.go`; **every emitted run code MUST have an explanation** (and no unused entries). Run codes/registry/selection live in the `workflow` package (`workflow/run_explain.go`: `RunExplanations`/`ExplanationCodesForRun`/`runExplainRegistry` + finding-code collectors); `Explanations()` merges step+run so `explain <code>`/`--list` cover both (44 codes); the render registry merges ONLY run + policy (a run carries no doctor codes). Per-step explain codes are stored UNEXPORTED on `WorkflowRunStep` (not in the JSON contract). `commands` stays handlers-only (`RunWorkflowRun` only wires `--explain` into `EmitRun`/the plan emitter). The support-bundle `explanations.json` stays doctor-scoped. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 5) — workflow run (read-only execution):** `workflow run --file` executes ONLY the supported read-only step types (`policy.check`/`policy.review`/`policy.baseline.compare`) by **reusing the existing policy/config/baseline helpers** (`policy.Evaluate`/`BuildReview`/`Compare`, `config.ParseLocal`) — it must NEVER run a shell command, execute the runbook `display` text, apply/import configs, enforce a policy, schedule, probe targets, or mutate the runtime/config/policy/baseline/report files. It runs only from a VALID plan (invalid → print the plan, exit 1, no step run, no `--out` file, like `workflow plan`/`runbook`). A `reportFrom` step consumes the producing step's **in-memory** report (no intermediate file); round-trip it through JSON (`snapshotFromReport`) so fingerprints match file-derived baselines, and **skip** the dependent step (failing the run) when the dependency produced no report. The `workflow` package must NOT import `client`/`commands` — the runtime-config fetch is **injected** (`RunDeps.RuntimeRules`); the command layer (`makeRuntimeRules`) owns the read-only `client.ExportConfig()` call + memoization + wrapping a connection error with `workflow.ErrRuntimeUnreachable` so runtime-unreachable exits **3** (matching `policy check --runtime`). Exit: 0 all passed (none skipped); 1 any failed/skipped, invalid plan, or `--out` write failure; 2 usage or unreadable/malformed workflow; 3 runtime unreachable — a referenced file unreadable/malformed DURING the run is a **step failure (1)**, not a usage error. The run report carries only what existing policy reports include (no raw configs/secrets). `commands` handlers-only; `--json --out` byte-identical. Adding a runnable step type = a new executor in `run.go` reusing a safe helper + coverage; do NOT add shell/arbitrary-command steps, mutation, enforcement, or a scheduler. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 4) — workflow runbook preview:** A workflow runbook (`workflow runbook --file`) is a **deterministic command PREVIEW, NOT execution** — it must NEVER execute a command, contact the runtime, read the files a step refers to (or inspect their contents), or mutate any file except `--out`. It is built **only from a VALID plan** (`BuildRunbook` after `BuildPlan` reports valid; `Result` is always `ready`); an invalid workflow prints the plan and exits 1 with **no runbook and no `--out` file** (mirror `workflow plan`). The runbook must **not invent file paths** — a `reportFrom` step uses an explicit `<report-from:step-id>` placeholder + a note. Each step exposes a canonical argv `Command` (source of truth) and a best-effort copy/paste `Display`; Portier must never shell-execute `Display`. Model/builder/renderer live in the `workflow` package (`workflow/runbook.go`), reuse `Parse`+`BuildPlan`, file writing reuses `output.WritePrettyJSON`; `commands` stays handlers-only (`RunWorkflowRunbook` validates then calls helpers — no command-generation logic in `commands`). `--json --out` byte-identical. Exit: 0 runbook, 1 invalid plan or `--out` write failure, 2 usage/unreadable/malformed — **no connection-failure (3)** code. Adding a step type = a new case in `runbookCommand` mapping to its real CLI command + coverage; do NOT add execution/scheduler/background automation here — execution is a deliberate later v1.11 slice. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 3) — workflow templates:** Built-in workflow templates (`workflow template <name>` / `--list`) must render to the **exact schema `workflow plan` accepts** (`schemaVersion: 1` + `name` + `steps`) — a rendered/`--out` template must be directly usable by `workflow plan --file` with no editing, guarded by a test that renders each template, `Parse`s it, and asserts `BuildPlan` is `valid`. Templates are **deterministic, sorted by name**, and live in the `workflow` package (`workflow/template.go`: registry, metadata, `WorkflowValue`/`DetailValue`/`PrintTemplateList`); definitions must NOT live in `commands` (handlers-only — parse flags, resolve template, call `output` for file writing). Templates compose ONLY the existing validated step types (`policy.check`/`policy.review`/`policy.baseline.compare`) and must **never introduce execution/scheduler/mutation/apply/import fields** (guarded by a field-allowlist test). The `--out` file is the **bare** workflow JSON (no wrapper) regardless of `--json`, via `output.WritePrettyJSON` (no parent-dir creation, no partial write); write failure → exit 1; unknown/missing/too-many template, missing `--out` value, or `--list`+name/`--out` → exit 2. Fully offline — no runtime contact, no step execution, no reading referenced input files, modifies nothing but `--out`. Mirrors the v1.10 `policy template` rule; keep the two aligned. Adding a template = a registry entry + render-to-valid-plan coverage; do NOT add a template-file loader, user template dir, or parameterized generator. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 2) — workflow validation codes & explanations:** Workflow step-validation codes (`workflow.step.*`) are **stable, operator-facing identifiers** — exactly one per validation outcome (15 today; `codeStepMissingPolicy` shared by policy.check + policy.review), the set is **locked by a test**, and **every code MUST have an explanation** (guarded by `workflow/explain_internal_test.go`). Do NOT invent codes for validations that do not exist. Explanation DATA lives in the `workflow` package (`workflow/explain.go`, `Explanations()`); the `explain` command composes domains via `explain.Merge(doctor, policy, workflow)` (dispatch-level, not domain logic in `commands`). Reuse the canonical `explain` package (one `Explanation` type + one renderer `explain.PrintInline`/`ForReport`) — no second schema, no registry framework. `workflow plan --explain` is **presentation only** — never changes which steps are validated, the plan body (steps/summary/result), or the exit code; the JSON `explanations` map is additive (`omitempty`, byte-identical without `--explain`; `--json --out --explain` byte-identical). `--explain` explains **invalid steps only** in BOTH human and JSON (a valid step needs no explanation; `workflow.step.valid` is in the registry for `explain <code>`/`--list` but never emitted into a plan's explanations map). Every `StepPlan` carries a stable `Code`. Workflow planning stays dry-run/offline (no step execution). Support-bundle `explanations.json` stays doctor-scoped. CLI stays a pure API client; no API/DTO/contract change.

**Durable rule (v1.11 Slice 1) — workflow plan dry-run:** A workflow is an **ordered sequence of EXISTING safe operations** in a small JSON file (`schemaVersion: 1` + non-empty `steps`); `portier workflow plan --file <file>` must be **deterministic, fully offline, and dry-run** — NEVER execute a step, contact the runtime, read/parse the files a step refers to, apply/import configs, enforce a policy, schedule anything, or mutate any file except `--out`. It **composes existing operations by reference** (`policy.check`/`policy.review`/`policy.baseline.compare`) — validate that each step names a known operation with coherent inputs; do NOT duplicate policy/doctor semantics or grow a scripting/expression/plugin engine. **Step types** are stable operator-facing identifiers (test any new one). Keep the **plan model separate** from policy/doctor (`Plan`/`StepPlan`/`PlanSummary` in package `workflow`). Validation is structural + reference-coherence only: `schemaVersion==1`, ≥1 step, unique non-empty ids, supported type with required fields, exactly-one-of where specified (`config`/`runtime`; `report`/`reportFrom`), `reportFrom` → an **earlier** step (acyclic by construction — no forward refs, no general cycle detector yet); **unknown fields rejected**. A missing **referenced** file must NOT fail the plan (planning never opens them). Output is symbol-free ASCII (`[VALID]`/`[INVALID]` + `Result: valid/invalid`); `--json` emits the full plan; `--out` writes byte-identical JSON. Exit: 0 valid, 1 invalid plan OR `--out` write failure, 2 usage or unreadable/malformed workflow (missing/unsupported `schemaVersion`, no steps) — **NO connection-failure (3)** code (never contacts runtime). Schema/parser/validator/model/render live in package `workflow`; `commands` stays handlers-only; dispatch is OFFLINE (no URL resolution). CLI stays a pure API client. No execution/scheduler/background automation/enforcement/apply/import/runtime mutation/target probing/UI panel/`allowUdp` in this slice — execution is a deliberate later v1.11 slice.

**Durable rule (v1.10 Slice 7) — policy baseline snapshots:** A policy baseline (`policy baseline create`/`compare`) is an **accepted snapshot of findings, NOT a config copy** — store only `{fingerprint, code, severity, title, message}` per finding plus `schemaVersion`/`createdAt`/`source`/`result`; never embed raw config, secrets, runtime host data, finding `details`, or explanations. Both commands are **fully offline and deterministic** — they operate ONLY on policy report JSON files (`policy check`/`policy review`, `--json`/`--out`), never contact the runtime, never probe, never mutate the inputs. **New findings fail; resolved-only changes do not** (compare result `failed` iff `summary.new > 0`; exit 1 iff new, else 0). Finding **fingerprints must be deterministic, source-independent, and test-covered** (rule-scoped `code|name|protocol|listenHost|listenPort`; duplicate-binding `code|protocol|listenHost|listenPort|<sorted rule names>`; fallback `code|message`) — read from the report's detail map, never using volatile rule IDs, identical for offline `--config` vs `--runtime`. The `policy.valid` marker is excluded from baselines/compare. Model/fingerprint/build/compare live in `policy/baseline.go`; file writing in `output`; `commands` handlers-only; `BuildBaseline` takes an injected clock (testable `createdAt`). Exit: create 0/1(write)/2(usage|malformed report); compare 0(no-new)/1(new)/2(usage|malformed baseline-or-report incl. unsupported baseline schemaVersion). CLI stays a pure API client; no API/DTO/contract change, no enforcement, no full config storage in the baseline.

**Durable rule (v1.10 CLI package layout):** In the Go CLI, **general model/result types and their pure offline logic do NOT live in `commands`** — `commands` holds only the `Run*` handlers, help text, and dispatch. Domain types+logic live in focused per-domain packages: `tools/cli/sources/config` (local config-file domain: `Rule`/`ParseLocal`/`Validate`/`Summary`/`FindDuplicateBindings`/`RuleDetail`/…), `tools/cli/sources/doctor` (doctor model + live/config check builders + `Explanation` registry + `Emit`/`PrintHuman`/`ExitCode`/`ReportPayload`), `tools/cli/sources/policy` (policy model + `Parse`/`Evaluate`/`PrintHuman`/`ExitCode`), `tools/cli/sources/planview` (config plan/diff/apply response **rendering**: `PrintPlan`/`PrintDiff`/`PrintApply`/`PlanExitCode`/`ApplyExitCode`), and generic helpers in `output` (`PrintJSON`/`WritePrettyJSON`/`PrintTable`/`PluralWord`/`PluralRule`). What intentionally STAYS in `commands`: the `Run*` handlers, per-command human printers (`printListHuman`/`printStatusHuman`/…), request-DTO mapping (`toConfigRules`/`build*Request`), single-command output structs (bundle/diagnostics/`ruleInfo`), and plumbing (`exitWithError`/`ResolveURL`/`ResolveRule`). Names are **idiomatic/de-stuttered** (`doctor.Report`, not `doctor.DoctorReport`; `config.Rule`, not `rawConfigRule`). Preserve the import DAG (`output`←all; `config` base; `doctor`/`policy` build on `config`; `commands` composes) — no cycles, and `config`/`doctor`/`policy` must NOT import `commands`. A new CLI command adds its `Run*` handler in `commands` and puts new model/logic in the right domain package — never back into `commands`. JSON struct tags are a contract: keep output byte-identical through any move (black-box tests assert verbatim; `validate:contract` 234/234). Adding/moving a CLI package requires updating `scripts/coverage-tools-cli.js` `coverpkg` so it is instrumented (else the gate under-counts). Prior references to `commands/{doctor,policy,config}.go` and `DoctorReport`/`PolicyReport`/`rawConfigRule` are **superseded** by this layout. CLI stays a pure API client.

**Durable rule (v1.10 Slice 1) — policy model & offline dry-run:** Policy evaluation (`portier policy check --config <file> --policy <file>`, `tools/cli/sources/commands/policy.go`) must be **deterministic, fully offline, dry-run** — NEVER contact the runtime, probe targets, scan host state, collect env/process/logs, or mutate runtime/config/**policy** files, unless a later slice explicitly changes this. **Policy finding codes** (`policy.*`) are **stable operator-facing identifiers** — don't rename casually; test new ones. Keep the model small and **separate from `DoctorReport`**: `PolicyFinding {code, severity(info|warning|error), title, message, details?}` → `PolicyReport {findings[], summary, result}` — no rule-engine/scripting/expression/plugin framework. The policy file is a small JSON schema (`schemaVersion` + a `rules` object of booleans `requireGroup`/`allowLanExposure`/`allowPrivilegedPorts`/`allowAutostart`/`forbidDuplicateBindings`); **reject unknown fields** (`DisallowUnknownFields`) and default omitted fields to a **permissive baseline** (empty policy permits everything). **Reuse the v1.9 config-doctor interpretations** (`toRuleDetail`/`findDuplicateBindings`/`ruleLabelName`, LAN = `0.0.0.0`, privileged = `<1024`) — no divergent interpretations; if sharing forces a big refactor, keep a small local helper but lock it to the doctor's with tests. Ordering is **observable**: per-rule findings in config file order (group → LAN exposure → privileged port → autostart), duplicate bindings last sorted proto→host→port — keep the ordering test green. Output is symbol-free ASCII (`[INFO]`/`[ERROR]`) + a `Result: passed/failed` line; `--json` emits the full report. Exit codes (v1.7 policy): `0` no violations, `1` ≥1 violation (or JSON-encode failure), `2` usage / unreadable / malformed config or policy (incl. unsupported `schemaVersion`) — invalid input is a CLI error, **not** a policy finding. **UDP is first-class** — NO `allowUdp`/`udpForbidden` rule or protocol allowlist/denylist unless a later feature deliberately adds one. CLI stays a pure client (no shared/server/service imports). No live enforcement, auto-remediation, scheduler, UI panel, or remote/auth/team behavior this slice.

**Durable rule (v1.9 Slice 10) — AI handoff prompt:** The doctor AI handoff prompt (`prompts/doctor.md`) must stay **safe and offline** — it must NOT tell users to paste secrets, tokens, private keys, env dumps, process lists, or logs (the doctor tools never collect those); it must tell the assistant to **treat the pasted doctor output as the source of truth** (don't invent runtime state); and it must NOT imply Portier sends data anywhere — Portier has **no AI integration, upload, or telemetry**, the user pastes locally-generated output into an assistant of their choice. Keep it distinguishing info/warning/error, strict failures, and operation failures, and asking for risk classification + prioritized next steps that **separate "safe now" from "needs admin/network/security approval."** Any future CLI helper (`--prompt`/`prompt`) must be fully offline (no config read, no runtime contact, no external send), print only the template, and not drift from the canonical doc. No AI API integration, telemetry, upload, or ticket creation.

**Durable rule (v1.9 Slice 9) — config doctor summary:** The config doctor `config` summary must be **deterministic and derived ONLY from the offline config** (no env/runtime/filesystem data). It must NOT change the `checks` array, exit semantics, `--strict`, or `--explain` (byte-identical with vs without it), and must stay **compact** (counts + per-group rollup, never a re-dump of full rule data — Slice 8 details own that). Groups sorted by trimmed name; empty/whitespace group → ungrouped; stable `protocols{tcp,udp}` shape. Place it as an **optional `omitempty` field on the shared `doctorReportJSON`**, set ONLY by config doctor (`doctorEmitOptions.config`) — the live `doctor` and support bundle leave it nil so their JSON stays byte-identical; no second wrapper. Present whenever the file parses (incl. empty/validation-failure); absent on read/parse failure. Enrich report metadata before adding check codes (Slice 9 added none). CLI stays a pure client; no API/DTO/contract change, no support-bundle change.

**Durable rule (v1.9 Slice 8) — config doctor details:** Config doctor finding `details` must be **deterministic, JSON-serializable, and derived ONLY from the offline config** — no OS env vars, process info, logs, filesystem data, runtime/live data, or probe results (only rule name/host/port/protocol/enabled/group, and id when present). **Sort collections deterministically** (duplicate `bindings` by protocol→host→port; rules within a binding/advisory in file order). **Enrich existing codes before adding new ones** (Slice 8 added none). Details must not change doctor semantics: keep exit behavior identical, keep LAN exposure as the `0.0.0.0` advisory (no `::`, no change to which rules are flagged), keep privileged as the deterministic `<1024` check (no `COMMON_PORTS` duplication). LAN/privileged advisories aggregate to **one check per kind** with a `rules` array (don't regress to one-per-rule; preserve single-rule titles). Put details on `DoctorCheckResult.Details` so they flow through `--json`/`--out`/`--explain`; `--explain` stays presentation-only; don't push config-doctor details into the live `doctor` or change support-bundle output. CLI stays a pure client; no API/DTO/contract change.

**Durable rule (v1.9 Slice 7) — doctor inline explanations:** Inline doctor explanations (`--explain`) MUST reuse the canonical Slice 3 explanation registry (`explanationsForReport`) — no duplicated strings, no second source. `--explain` must **not change which checks run**, the `checks`/`summary`/`result`, or any **exit code** (presentation only — guard with an explain-vs-non-explain JSON equality test). Explain **only codes present in the report**, deduped; unknown codes degrade safely (human `(no explanation available)`, JSON omitted) and an internal test must keep proving every emittable code is explainable. The JSON `explanations` map is **additive/`omitempty`** on `doctorReportJSON`, set only with `--explain`, and must not change the existing shape (non-explain output stays byte-identical). `--out --explain` writes the same payload as stdout. Do NOT add inline explanations to the support bundle's `doctor.json`/`doctor.txt` (it ships `explanations.json`). Keep doctor emit flags in `doctorEmitOptions`. CLI stays a pure client; no API/DTO/contract change; `explain <code>` behavior unchanged.

**Durable rule (v1.9 Slice 6) — doctor support bundle:** `support-bundle --out <dir>` must **reuse the doctor report JSON schema** (`doctorReportJSON`) for `doctor.json` and the existing doctor builder/renderer/explanation-registry/`writePrettyJSON` helpers — no second schema, no duplicated doctor logic. **Read-only**: contact the runtime but never mutate runtime/config, never probe targets. **Safety**: a bundle must NOT contain OS env vars, process lists, logs, tokens, or arbitrary filesystem scans (rule names/hosts/ports + runtime URL + output path are allowed). A **runtime-unreachable bundle is still created** (manifest + `doctor.json` with `runtime.unreachable` + `doctor.txt` + `explanations.json`; omissions → manifest warnings) and exits 1. Never silently overwrite: a non-empty dir or non-directory path is refused (exit 1); an empty dir is used; a missing dir is created. Filesystem/write failure → operation failure exit 1; missing `--out` → usage exit 2. Write `manifest.json` last. Keep it a directory of plain JSON/text (zip deferred); no redaction framework, log/process/env collection, server-side endpoint, or UI panel. Distinct from the single-file `diagnostics export`. CLI stays a pure client; no API/DTO/contract change.

**Durable rule (v1.9 Slice 5) — doctor report export:** Doctor `--out` export must write the **same JSON schema as `--json`** (the `doctorReportJSON` `{checks, summary, strict, result}` shape) — no second schema — and reuse the existing `writePrettyJSON` helper (no near-duplicate writer, no parent-dir creation, no partial write on marshal failure). Export must **not mutate** runtime/config and must not change which checks run or the diagnostic exit semantics. A **file-write failure → exit 1** (operation failure, stderr), overriding the report's exit code; a missing `--out` value → exit 2. `--out` writes the file regardless of `--json`; keep stdout pure JSON under `--json` (no confirmation line) and print `Report written to <file>` only in human mode. The stdout JSON (`--json`) and the file contents must be byte-identical (build the payload once). For positional doctor commands (`config doctor`) flags precede the file. CLI stays a pure client; no API/DTO/contract change; no support-bundle/redaction/replay scope here.
...
**Durable rule (v1.13 Slice 1) — standalone replay tool:** Replay is a **separate offline tool beside the CLI** (`tools/replay/`, binary `replay`, its own Go module `portier/replay`) — NEVER a `portier`/`portier workflow replay` subcommand inside `tools/cli`, and it must stay independently buildable/testable (`build:replay`/`test:replay`/`validate:replay`). It must analyze **saved artifacts only** and be **strictly offline and read-only**: it must NEVER execute workflows, contact the runtime, read the config/policy/baseline/report files an artifact refers to (for a support-report bundle directory read ONLY the bundle's own `manifest.json`/`report.json`/`explanations.json`, never a referenced file), mutate inputs, apply/import configs, enforce policy, schedule jobs, run shell commands, upload anything, or collect logs/environment/process data. **Input detection is by JSON/manifest shape, not filename.** The replay plan is a **capability description** ("what offline analysis can this artifact support?") — it must NOT replay by re-executing anything, and every plan must include the four always-unavailable safety items (execution, runtime probing, referenced-file reread, mutation/enforcement). Replay **schemas are LOCAL tool schemas** (`schemaVersion: 1` on the plan), NOT a REST/API/contract surface unless deliberately promoted later; they must not affect `validate:contract` (stays 234/234). The replay tool must NOT import `tools/cli` (any package), the runtime client, or server/service/shared — define **minimal local DTOs** for detection rather than coupling to the CLI module. Keep detection/plan/render pure and unit-tested with a thin `main.go` dispatcher; `--json` stdout and the `--out` file must stay byte-identical (one marshaller). Exit codes: `0` success; `1` an output-write/JSON-encode failure; `2` a usage error or an unreadable/malformed/unsupported artifact; **no exit `3`** (never contacts a runtime). Leave the existing CLI unchanged unless a clearly-useful, low-risk shared-helper extraction is made. Later v1.13 slices add the actual analyses (timeline reconstruction, distributions, correlation, incident reports) on top, under the same safety boundary.

**Durable rule (v1.13 Slice 2) — replay analyze:** `replay analyze` is **offline artifact analysis ONLY** — analyze the supplied artifact deterministically from its OWN contents, parsing **only** the supplied `--from` JSON file (or, for a support-report bundle directory, ONLY the bundle's own `manifest.json`/`report.json`/`explanations.json`). It must NEVER execute workflows, contact the runtime, read the config/policy/baseline/report files an artifact refers to (reading a report's EMBEDDED report object is fine; opening a referenced PATH is not), mutate inputs, apply/import, enforce policy, schedule, run shells, upload, or collect logs/env/process data. **Reuse Slice 1 detection** (`DetectJSON`/`DetectBundleManifest`) to classify, then decode analysis-specific structs — no second classifier, no `tools/cli`/workflow coupling (minimal local DTOs). The **replay analysis schema is a LOCAL tool schema** (`schemaVersion: 1`, package `core`), **separate from the replay-plan schema**; neither is a REST/API/contract surface (`validate:contract` stays 234/234). Analysis must be **deterministic** (code/workflow counts by count desc then key asc; sorted ID lists; stable insights; history code counts = **runs containing** each code, deduped per run — matching v1.12 history stats). Output slices serialize as `[]` not `null`; `--json` stdout and the `--out` file are byte-identical (one marshaller). Exit codes: `0` analysis created; `1` an output-write/JSON-encode failure; `2` a usage error or an unreadable/malformed/unsupported artifact; **no exit `3`** (never contacts a runtime). Add focused tests for every new analyzer/helper in the same slice; keep replay-package coverage ≥95%. The CLI stays the operational artifact **producer**; replay is the offline artifact **analyzer** — do NOT migrate/duplicate CLI commands or add execution/rerun/scheduler/runtime-probing/history-import/upload. `replay plan` and the whole CLI stay unchanged.

**Durable rule (v1.13 Slice 3) — replay tool layout & binary name:** The standalone replay tool's binary is **`replay`** (`replay.exe` on Windows), built via `go -C tools/replay build -o build/replay ./sources` — do NOT reintroduce `portier-replay` (it survives only in historical changelog entries). The Go layout is fixed: **`tools/replay/sources/core`** = domain logic (artifact detection, replay plan/analysis models, builders, renderers); **`tools/replay/sources/commands`** = CLI dispatch, argument parsing, command runners (exported `Run`); **`tools/replay/sources/main.go`** = a thin entry point calling `commands.Run` only. There is no `sources/replay` package. Layering is one-way — **`commands` may import `core`; `core` must NEVER import `commands`** — and neither may import `tools/cli`, the runtime client, or server/service/shared. New replay commands add a runner in `commands` with model/detection/analysis logic in `core` (or a new domain package); do NOT push domain logic into `commands`/`main.go` or fold replay into `tools/cli`. Structural/naming only: no change to replay behavior, output, exit codes, schemas, or the offline/read-only boundary; `validate:contract` stays 234/234.

**Durable rule (v1.13 Slice 4) — replay timeline:** `replay timeline` is **offline artifact analysis ONLY** — reconstruct a deterministic ordered view from the supplied artifact's OWN contents, parsing **only** the supplied `--from` JSON file (or, for a bundle directory, ONLY the bundle's own `manifest.json`/`report.json`). It must NEVER execute workflows, contact the runtime, read the config/policy/baseline/report files an artifact refers to (an EMBEDDED report object is fine; a referenced PATH is not), mutate inputs, apply/import, enforce policy, schedule, run shells, upload, or collect logs/env/process data. **Reuse existing detection** (`DetectJSON`/`DetectBundleManifest`) then decode timeline-specific structs in `core` — no second classifier, no `tools/cli`/workflow coupling. The timeline must **clearly distinguish SYNTHETIC reconstruction events from SAVED events** (an always-serialized `synthetic` boolean per event + a `synthetic` summary count + `*-start`/`*-result`/`*-end` lifecycle markers) and must **NEVER invent timestamps** — order is saved order only (1-based `Order`); history stays newest-first (tested). The **replay timeline schema is a LOCAL tool schema** (`schemaVersion: 1`, package `core`), **separate from the plan and analysis schemas**; none is a REST/API/contract surface (`validate:contract` stays 234/234). `events` serializes as `[]` not `null`; `--json` stdout and the `--out` file are byte-identical (one marshaller). Exit codes: `0` created; `1` an output-write/JSON-encode failure; `2` a usage error or an unreadable/malformed/unsupported artifact; **no exit `3`**. Add focused tests for every new builder/helper. **The replay module must finish at ≥95% coverage (acceptance requirement), measured module-wide via `go -C tools/replay test -coverpkg=./... ./...`** — restore it with MEANINGFUL tests for real branches/error cases (e.g. a `failWriter` stdout-write-failure test), never coverage-gaming; the only acceptable uncovered code is structurally-unreachable defensive branches and the `main()` `os.Exit` wrapper (documented). The CLI stays the operational artifact **producer**; replay is the offline **analyzer** — do NOT migrate/duplicate CLI commands or add execution/rerun/scheduler/runtime-probing/history-import/upload. `replay plan`/`analyze` and the whole CLI stay unchanged.

**Durable rule (v1.13 Slice 5) — replay compare:** `replay compare` is **offline artifact comparison ONLY** — parse **only** the two supplied `--left`/`--right` artifacts (and, for a bundle directory, ONLY its own `manifest.json`/`report.json`). It must NEVER execute workflows, contact the runtime, read the config/policy/baseline/report files an artifact refers to (an EMBEDDED report object is fine; a referenced PATH is not), mutate either input, apply/import, enforce policy, schedule, run shells, upload, or collect logs/env/process data. **Reuse existing detection + the single-parse `compareFacts` extractor** (`DetectJSON`/`DetectBundleManifest` + timeline raw structs + shared helpers) — no second classifier, no third raw-struct family, no `tools/cli`/workflow coupling. **Mixed-kind comparisons MUST be clearly LIMITED** (a `kind.mismatch` change + a `replay analyze` pointer, no `sets`) — never a crash, never a false cross-kind-equivalence claim (only undetectable/unparseable input is exit 2). Output MUST be **deterministic**: changes in a fixed semantic order (never raw map iteration; iterate sorted key unions for distribution deltas), set values sorted ascending, `diffSets` non-nil sorted added/removed/unchanged (serialize `[]` not `null`). The **replay compare schema is a LOCAL tool schema** (`schemaVersion: 1`, package `core`), **separate from plan/analysis/timeline**; none is a REST/API/contract surface (`validate:contract` stays 234/234). `--json` stdout and the `--out` file are byte-identical (one marshaller). Exit codes: `0` created; `1` an output-write/JSON-encode failure; `2` a usage error (missing `--left`/`--right`) or an unreadable/malformed/unsupported artifact; **no exit `3`**. Keep the replay module at **≥95% module-wide coverage** with meaningful tests — enforced by `npm run validate:coverage:replay` (the `replay` gate in `scripts/validate-coverage.js`, fed by the `scripts/coverage-tools-replay.js` reporter; a hard 95% gate independent of the CLI/workspace gates). The CLI stays the operational artifact **producer**; replay is the offline **analyzer** — do NOT migrate/duplicate CLI commands or add execution/rerun/scheduler/runtime-probing/history-import/upload. `replay plan`/`analyze`/`timeline` and the whole CLI stay unchanged.

**Durable rule (v1.13 Slice 6) — replay explain:** `replay explain` is **offline code explanation ONLY** — parse **only** the supplied artifact (`--from`: workflow run/plan report, history export, bundle dir's own `manifest.json`/`report.json`, or a replay analysis/timeline/compare JSON) or operate on the local registry (`--code`/`--list`). It must NEVER execute workflows, contact the runtime, read the config/policy/baseline/report files an artifact refers to (an EMBEDDED report object is fine; a referenced PATH is not), mutate the input, apply/import, enforce policy, schedule, run shells, upload, or collect logs/env/process data. **Reuse existing extraction** (`factsFromJSON`/`bundleFacts` + `replayOutputCodes`) — no new classifier/raw-struct family, no `tools/cli`/runtime coupling. **Unknown codes MUST be preserved + marked `known:false`** (never dropped/errored); a **no-code artifact is success (exit 0)**. Codes deduped + **sorted ascending**. The registry is a **small LOCAL registry in `core`** (`workflow.step.*`/`workflow.run.*`/`policy.*`) — never import the CLI explain registry; doctor/runtime codes out of scope (reported unknown); keep it integrity-tested (Code==key, Known true, valid severity, non-empty fields). The **explain schema is a LOCAL tool schema** (`schemaVersion: 1`, package `core`), **separate from plan/analysis/timeline/compare**; none is a REST/API/contract surface (`validate:contract` stays 234/234). `explanations` serializes `[]` not `null`; `--json` stdout and the `--out` file byte-identical. Exactly one of `--from`/`--code`/`--list`. Exit codes: `0` produced (incl. no codes); `1` an output-write/JSON-encode failure; `2` a usage error or an unreadable/malformed/unsupported artifact; **no exit `3`**. Keep the replay module at **≥95% module-wide coverage** with meaningful tests — enforced by `npm run validate:coverage:replay` (the `replay` gate in `scripts/validate-coverage.js`, fed by the `scripts/coverage-tools-replay.js` reporter; a hard 95% gate independent of the CLI/workspace gates). The CLI stays the operational artifact **producer**; replay is the offline **analyzer** — do NOT migrate/duplicate CLI commands or add execution/rerun/scheduler/runtime-probing/history-import/upload. `replay plan`/`analyze`/`timeline`/`compare` and the whole CLI stay unchanged.
...
**Durable rule — coverage-gate tightening (opportunistic ratchet):** Tighten coverage gates **opportunistically**. When a slice raises **meaningful, stable** coverage and a component is **safely above** its current minimum gate, raise **that component's** gate **modestly** in the **same slice** (e.g. 95 → 96, not a big jump; only the touched component, never all at once). **Never lower a gate.** Only raise when the component stays **safely above the new threshold after re-running** the relevant `validate:coverage:*` (confirm PASS). Do **not** raise on a **brittle/platform-specific/one-off** spike — if the uncovered remainder is legitimate defensive/platform/error paths that vary by OS or run, leave the gate and document why. On any gate change, update **all** surfaces together: `scripts/validate-coverage.js` (dated comment), the gate/baseline prose in `CLAUDE.md`/`AGENTS.md`, the checklist entry, and the changelog. **Report** previous gate, new gate, actual coverage, and why it's safe — and **if a gate could have been raised but wasn't, explain why**. The numeric gate is only the **floor**; the goal remains **meaningful coverage** of important behavior, contracts, error paths, rollback paths, and guardrails — tested or explicitly documented as structurally unreachable.
...
v1.1 focuses on distribution and native OS service installers. The v1.1 scope, platform strategy, install layouts, artifact targets, and implementation slices are defined in `docs/installer.md`.

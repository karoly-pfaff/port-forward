...
// container (app.App) and the chi API router. Every API route is served by the
// chi router; an unmatched request returns the generic /api 404 envelope via
// writeAPINotFound. Feature handlers reach the rule manager through
// h.app.Manager — app.App is the single dependency container.
type Handler struct {
	app             *app.App
	staticAvailable bool

	// apiRouter is the chi router owning all API routes. It only ever serves
	// requests under /api; its NotFound/MethodNotAllowed handlers emit the generic
	// /api 404 envelope (writeAPINotFound) — never a 405.
...
// than a 405 — Portier's intentional 404-not-405 contract for the /api surface.

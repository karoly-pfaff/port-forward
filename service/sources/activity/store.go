...
func (s *Store) List(params ListParams) []ActivityEvent {
	...
	// Size the pre-allocation by the events actually present (intrinsically
	// bounded by MaxEvents via Add) rather than the user-supplied limit; the
	// loop below stops at limit regardless.
	result := make([]ActivityEvent, 0, len(snapshot))
